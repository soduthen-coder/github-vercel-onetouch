...
터미널은 **글자로 컴퓨터에게 시키는 앱**입니다. 이미 컴퓨터에 들어 있습니다.
무섭게 생겼지만 할 일은 한 줄 붙여넣고 엔터, 그게 전부입니다.

**윈도우**

1. 키보드 `윈도우 키` 를 누릅니다
2. `git bash` 라고 입력합니다
3. 나오는 **Git Bash** 를 클릭합니다

git 을 설치하면 함께 깔립니다. 안 보이면 아직 설치 전입니다.
...
> **붙여넣기가 안 되면**
> 윈도우 Git Bash 에서는 `Ctrl+V` 가 안 먹습니다.
> 창 안에서 **마우스 오른쪽 버튼**을 누르면 붙여넣어집니다. 맥은 `⌘+V` 로 됩니다.

---

## 시작하기

열린 터미널 창에 아래를 한 줄씩 붙여넣고 엔터를 누릅니다.
...
터미널이 영 어렵다면 내려받은 폴더에서 `start.cmd` 를 **더블클릭**해도 됩니다.
(윈도우 전용)
...
| 붙여넣기가 안 됩니다 | Git Bash 에서는 마우스 오른쪽 버튼을 누르세요 |
| Git Bash 가 안 보입니다 | git 이 아직 설치되지 않은 것입니다 |
